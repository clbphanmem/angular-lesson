...
class Tool
{
    ...
    public static function pagination($totalPage = 0, $perPage = 10, $currentPage = 1, $range = 2, $linkCallback)
    {
        ...
        if ($currentPage - 1 > 0) {
            $data['render'] .= sprintf('<li><a href="%s">&laquo;</a></li>', is_callable($linkCallback) ? $linkCallback($currentPage - 1) : '', $currentPage);
        }
        ...
        if ($currentPage + 1 > 0 && $currentPage < $totalPage) {
            $data['render'] .= sprintf('<li><a href="%s">&raquo;</a></li>', is_callable($linkCallback) ? $linkCallback($currentPage + 1) : '', $currentPage);
        }
        $data['render'] .= '</ul></nav>';
